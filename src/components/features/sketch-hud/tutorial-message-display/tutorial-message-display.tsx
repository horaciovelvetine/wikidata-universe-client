...
export const TutorialMessageDisplayTutorialMessageDisplayProps = ({ sketchRef, setIsTutorialSketch }: TutorialMessageDisplayProps) => {
  ...
  const [instruction, setInstruction] = useState(tutorialSketchRef.state.tutorialInstruction());

  useEffect(() => {  // tell sketch how to update react's display of tutorial messag-ing
  ...
  const handleAdvanceTutorialClick = async () => {
    const res = await getTutorial(tutorialSketchRef.getNextSlideNumber().toString())
    tutorialSketchRef.state.updateTutorialState(res);
  };

  const handleReverseTutorialClick = async () => {
    const res = await getTutorial(tutorialSketchRef.getPrevSlideNumber().toString())
    tutorialSketchRef.state.updateTutorialState(res);
  };
    ...
    <div id={ID('container')}>
      <div id={ID('layout')}>
        <div id={ID('exit-container')}>
          <img id={ID('close-icon')} src={Close} onClick={handleExitTutorialClick} />
        </div>
        <p id={ID('body')}>{bodyMsg}</p>
        <div id={ID('forward-back-container')}>
          <img id={ID('back-icon')} src={ArrowLeft} onClick={handleReverseTutorialClick} />
          <p>{instruction}</p>
          <img id={ID('forward-icon')} src={ArrowRight} onClick={handleAdvanceTutorialClick} />
        </div>
      </div>
    </div>
  )
