...
export const RelatedEdgesInfo: FC<RelatedEdgesInfoProps> = ({ sketchRef }) => {
  ...
  useEffect(() => { // Let sketch know about curSelected subscriber
    sketchRef.state.addCurSelectedSubscriber(setCurSelectedRef);
  })

  useEffect(() => { // setup related edges, followup effect places rel-edges on-screen
  ...
  return (
    <div id={ID('container')} ref={containerRef}>
      ...
      <div id={ID('details-container')} onWheel={(e) => e.stopPropagation()}>
        <ul id={ID('list')}>
          {curSelectedRef && relEdges.map(edge => <RelatedEdgeItem {...{ edge, sketchRef, curSelectedRef, relEdges }} key={`edge-${edge.srcId}-${edge.tgtId}`} />)}
        </ul>
      </div>
    </div>
  )
