...
export const SketchDetailsSummary = ({ sketchRef }: SketchDetailsSummaryProps) => {
  ...
  const [topicCount, setTopicCount] = useState(sketchRef?.state.topicCount() || 0);
  const [statementCount, setStatementCount] = useState(sketchRef?.state.topicCount() || 0);
  const [camPosition, setCamPosition] = useState(new Point3D());
  const [camFocus, setCamFocus] = useState(new Point3D());

  useEffect(() => { // attach interval to update camera position & focus intervals...
    ...
    sketchRef.state.addShowSketchDetailsSummarySubscriber(setShowSketchDetailsSummary);
    ...
    const updateCamValues = () => {
      ...
      setCamFocus(new Point3D({ x: cam.centerX, y: cam.centerY, z: cam.centerZ }))
    }
