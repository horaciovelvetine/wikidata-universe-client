...
export const ReSearchInput = ({ sketchRef, setInitSketchData }: ReSearchInputProps) => {
  ...
  useEffect(() => { // tell sketch how to update query...
    sketchRef.state.addQuerySubscriber(setQuery);
    moveReSearchInputInView(ReSearchCont);
  })

  useEffect(() => { // hide input when screen to small...
  ...
  return (
    <div id={ID('container')} ref={ReSearchCont}>
      ...
      <form id={ID('form')} onSubmit={handleNewSearchSubmit}>
        <input id={ID('input')} type='text' value={query} onChange={handleSearchInputChange} ref={InputRef} />
      </form>
    </div>
  )
}