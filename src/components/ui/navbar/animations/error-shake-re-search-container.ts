 ...
 */
export function errorShakeReSearchContainer(inputRef: RefObject<HTMLInputElement>, containerRef: RefObject<HTMLDivElement>) {
  const input = inputRef.current
  const cont = containerRef.current
  if (input && cont) {
    input.style.transition = "border-color 0.82s, color 0.82s, box-shadow 0.82s, caret-color 0.82s";
    cont.style.animation = "horizontal-shake 0.82s cubic-bezier(.36,.07,.19,.97) both";
