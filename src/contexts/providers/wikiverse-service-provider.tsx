...
const WikiverseApiContext = createContext<ServiceProvider | undefined>(
  undefined
);
...
export const WikiverseServiceProvider = ({
  ...
  useEffect(() => {
    // check API status on mount (assumes offline by default)
  ...
  const INV_STATUS_TXT = (status: number) =>
    `HTTP error, invalid status ${status}`;
  ...
  const getQueryData = useCallback(
    async (query: string): Promise<WikiverseServiceResponse> => {
      setIsLoading(true);

      const response = await fetch(
        `${baseUrl}/query-data?${new URLSearchParams({ query }).toString()}`
  ...
  const checkServiceAvailability = useCallback(async (): Promise<boolean> => {
    setIsLoading(true);
    const response = await fetch(`${baseUrl}/current-status`).finally(() =>
      setIsLoading(false)
    );
  ...
  const getTutorial = useCallback(
    async (target: string): Promise<WikiverseServiceResponse> => {
      setIsLoading(true);

      const response = await fetch(
        `${baseUrl}/tutorial?${new URLSearchParams({ target }).toString()}`
      ).finally(() => setIsLoading(false));
      const json = await response.json();
